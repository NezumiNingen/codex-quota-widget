...
一个本地 Codex 插件，用一张简洁的卡片展示额度剩余、套餐和重置时间。

## 真实数据边界

实时同步通过本机 Codex app-server 的只读账户接口获取套餐、额度、重置时间、Credits 和 token 用量。它不会读取、保存或显示登录令牌和邮箱；同步结果仅保存在 `~/.codex/codex-quota-live.json`。
...
安装后，在新任务中说“查看我的 Codex 额度”。首次使用可说：
